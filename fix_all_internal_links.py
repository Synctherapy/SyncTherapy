...
def ensure_trailing_slash(url):
    if not url.endswith('/') and '#' not in url and '?' not in url and not re.search(r'\.[a-zA-Z0-9]+$', url):
        return url + '/'
    return url

rmap = get_redirects_map()

# Regex to match href="..." or markdown links ](...)
# We want to process the URL inside the link
