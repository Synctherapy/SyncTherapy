...
export function MassageTherapyServices() {
    return (
        <section className="py-24 lg:py-32 relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-blue-100/40 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none mix-blend-multiply opacity-70 animate-blob will-change-transform" aria-hidden="true" />
            <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-purple-100/40 rounded-full blur-3xl translate-y-1/3 -translate-x-1/3 pointer-events-none mix-blend-multiply opacity-70 animate-blob animation-delay-2000 will-change-transform" aria-hidden="true" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mx-auto">
                        {modalities.map((service, index) => (
                            <ScrollReveal key={index} delay={index * 0.05} className="h-full">
                                <Link href={service.href} className="block h-full cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 rounded-xl">
                                    <GlassCard
                                        variant="default"
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-2 flex-grow">
                                            ...
                                            <div className="pt-4 mt-auto">
                                                ...
                                                >
                                                    {service.cta} <ArrowRight className="ml-1 w-3 h-3" aria-hidden="true" />
                                                </span>
                                            </div>
                                        </div>
                                    </GlassCard>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {treatableConditions.map((condition, index) => (
                                <ScrollReveal key={index} delay={index * 0.03}>
                                    <Link href={condition.href} className="block group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2 rounded-xl">
                                        <div className="bg-white/50 hover:bg-white border border-white/60 hover:border-green-200 shadow-sm hover:shadow-md rounded-xl p-4 transition-all duration-300 flex items-center justify-between">
                                            <span className="text-lg font-medium text-slate-700 group-hover:text-green-700 transition-colors flex items-center">
                                                <CheckCircle2 className="w-5 h-5 mr-3 text-green-500/50 group-hover:text-green-600 transition-colors" aria-hidden="true" />
                                                {condition.text}
                                            </span>
                                            <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-green-600 transform group-hover:translate-x-1 transition-all" aria-hidden="true" />
                                        </div>
                                    </Link>
                                </ScrollReveal>
                            ))}
