...
export const getContentBySlug = cache(async (slugStr: string) => {
    // 1. Try to find the file using the full URL path (e.g. "services/massage")
    const pagePath = path.join(contentDirectory, 'pages', `${slugStr}.md`);
    const postPath = path.join(contentDirectory, 'posts', `${slugStr}.md`);
